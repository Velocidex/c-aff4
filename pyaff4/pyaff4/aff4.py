...
class Image(AFF4Object):
    ...
    def getDTB(self):
        ...
        if len(kaslrSlide) == 1 and long(kaslrSlide[0]) != 0:
            # it is Mac OS
            return 0
        else:
            try:
                dtb = self.resolver.QuerySubjectPredicate(self.urn, lexicon.standard.memoryPageTableEntryOffset).next()
                return long(dtb)
            except:
                # some early images generated by Rekall don't container CR3
                return 0
