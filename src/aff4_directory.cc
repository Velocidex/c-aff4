...
AFF4ScopedPtr<AFF4Directory> AFF4Directory::NewAFF4Directory(
  ...
  string root_path = root_urn.ToFilename();
  struct stat s;
  XSDString mode;
...
AFF4ScopedPtr<AFF4Stream> AFF4Directory::CreateMember(URN child) {
  ...
  string relative_path = urn.RelativePath(child);
  ...
  string filename = member_name_for_urn(child, urn, false);
...
AFF4Status AFF4Directory::LoadFromURN() {
  ...
  if (desc.get()) {
    string urn_string = desc->Read(1000);
...
AFF4Status AFF4Directory::RemoveDirectory(const string &path) {
  ...
  if (dir != NULL) {
    ...
    while ((ent = readdir(dir)) != NULL) {
      ...
      string full_path = dirname + ent->d_name;
